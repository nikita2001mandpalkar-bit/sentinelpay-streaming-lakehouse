...
from src.quality.contracts.bank_accounts_contract import BANK_ACCOUNTS_CONTRACT
...
def load_silver_dataframe(spark):
    return (
        spark.read
        .format("delta")
        .load(BANK_ACCOUNTS_CONTRACT["silver_path"])
    )


def validate_required_columns(dataframe) -> None:
    actual_columns = set(dataframe.columns)
    required_columns = set(BANK_ACCOUNTS_CONTRACT["required_columns"])
...
def get_batch_definition(context):
    ...
    asset_name = "silver_bank_accounts"
...
def build_expectation_suite():
    return gx.ExpectationSuite(
        name="bank_accounts_suite",
        expectations=[
            gx.expectations.ExpectColumnValuesToNotBeNull(column="bank_account_id"),
            gx.expectations.ExpectColumnValuesToNotBeNull(column="customer_id"),
            gx.expectations.ExpectColumnValuesToNotBeNull(column="bank_name"),
            gx.expectations.ExpectColumnValuesToNotBeNull(column="ifsc_code"),
            gx.expectations.ExpectColumnValuesToBeInSet(
                column="account_type",
                value_set=BANK_ACCOUNTS_CONTRACT["allowed_account_types"],
            ),
            gx.expectations.ExpectColumnValuesToBeInSet(
                column="is_primary",
                value_set=BANK_ACCOUNTS_CONTRACT["allowed_is_primary_values"],
            ),
            gx.expectations.ExpectColumnValuesToBeInSet(
                column="account_status",
                value_set=BANK_ACCOUNTS_CONTRACT["allowed_account_statuses"],
            ),
            gx.expectations.ExpectColumnValuesToBeUnique(
                column=BANK_ACCOUNTS_CONTRACT["business_key"],
            ),
        ],
    )
...
def run_expectation_check(dataframe):
    ...
    validation_definition = gx.ValidationDefinition(
        name="bank_accounts_validation_definition",
...
def build_result_summary(validation_result):
    ...
    return {
        "dataset_name": BANK_ACCOUNTS_CONTRACT["dataset_name"],
...
def build_failed_rows_dataframe(dataframe):
    failed_reason = concat_ws(
        ", ",
        when(col("bank_account_id").isNull(), lit("bank_account_id_null")),
        when(col("customer_id").isNull(), lit("customer_id_null")),
        when(col("bank_name").isNull(), lit("bank_name_null")),
        when(col("ifsc_code").isNull(), lit("ifsc_code_null")),
        when(
            ~col("account_type").isin(BANK_ACCOUNTS_CONTRACT["allowed_account_types"]),
            lit("invalid_account_type"),
        ),
        when(
            ~col("is_primary").isin(BANK_ACCOUNTS_CONTRACT["allowed_is_primary_values"]),
            lit("invalid_is_primary"),
        ),
        when(
            ~col("account_status").isin(BANK_ACCOUNTS_CONTRACT["allowed_account_statuses"]),
    ...
    return (
        ...
        .withColumn("dataset_name", lit(BANK_ACCOUNTS_CONTRACT["dataset_name"]))
        .filter(col("failed_reason") != "")
    )


def build_duplicate_rows_dataframe(dataframe):
    business_key = BANK_ACCOUNTS_CONTRACT["business_key"]
    ...
    return (
        ...
        .withColumn("failed_reason", lit("duplicate_bank_account_id"))
        .withColumn("failed_at", current_timestamp())
        .withColumn("dataset_name", lit(BANK_ACCOUNTS_CONTRACT["dataset_name"]))
    )
...
def main() -> None:
    logger.info("=" * 60)
    logger.info("Starting bank accounts quality validation...")
    logger.info("=" * 60)

    spark = create_spark_session("SentinelPay Quality Bank Accounts")
    ...
    try:
        ...
        failed_df = (
            ...
            .dropDuplicates(["bank_account_id", "failed_reason"])
        )

        write_quarantine_dataframe(
            dataframe=failed_df,
            output_path=BANK_ACCOUNTS_CONTRACT["quarantine_path"],
        )

        write_result_summary(
            spark=spark,
            summary=summary,
            output_path=BANK_ACCOUNTS_CONTRACT["result_path"],
        )
        ...
        logger.exception("bank_accounts quality validation failed.")
        raise
